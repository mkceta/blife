...
export function BottomNav() {
    ...
    const [user, setUser] = useState<NavUserProfile | null>(null)
    ...
    // Removed mounted check to allow SSR rendering and avoid layout shift
    // User state will hydrate client-side, showing generic icon initially which is fine
    ...
    return (
        <div className={cn(
            "fixed bottom-0 left-0 right-0 z-50 glass-strong pb-safe md:hidden shadow-[0_-5px_20px_rgba(0,0,0,0.05)]",
            ...
            <nav className="grid grid-cols-5 items-center h-14 max-w-screen-xl mx-auto px-0 w-full">
                {blifeItems.map((item) => {
                    ...
                    let isActive = currentPath.startsWith(item.href)

                    // Prevent 'Home' (/market) from hijacking 'Sell' (/market/new)
                    if (item.href === '/market' && currentPath.startsWith('/market/new')) {
                        isActive = false
                    }

                    // Exact match for root simple paths if needed, but above covers the conflict
                    ...
                    if (item.isMiddle) {
                        return (
                            ...
                            >
                                <div className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full p-3 shadow-lg hover:shadow-primary/25 transition-all active:scale-95">
                    ...
                    return (
                        <button
                            ...
                            aria-label={item.label}
                            className={cn(
                                "flex flex-col items-center justify-center space-y-0.5 w-full h-full transition-all duration-200 active:scale-90",
                                    ...
                                    <>
                                        {isActive && item.label === 'Comunidad' ? (
                                            ...
                                            <item.icon className={cn(
                                                ...
                                                isActive
                                                    ? (item.icon === Search || item.icon === PlusCircle)
                                                        ? "stroke-[2.5]"
