...
function ChatContent() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            const { error: notifError } = await supabase
                ...
                .update({ read: true })
                .eq('user_id', user.id)
                .ilike('link', `%${threadData.id}%`) // Simplified matching
                .eq('read', false)
    ...
    if (loading) return <div className="flex items-center justify-center h-full">Cargando chat...</div>
    ...
    return (
        <div className="flex flex-col h-full w-full">
            <ChatHeader
                otherUser={otherUser}
                item={item ? { id: item.id, title: item.title, price } : null}
                itemType={itemType}
                thread={thread}
