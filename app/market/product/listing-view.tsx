...
import { ChevronLeft, Share2, ShieldCheck, Eye, ChevronRight, MoreVertical, Heart } from 'lucide-react'
import { formatTimeAgo } from '@/lib/format'
import { Carousel, CarouselContent, CarouselItem } from '@/components/ui/carousel'
import { ListingStatusControls } from '@/features/market/components/listing-status-controls'
...
import { RelatedListings } from '@/features/market/components/related-listings'
import { ProductActions } from '@/features/market/components/product-actions'
import { calculateTotalWithFees } from '@/lib/pricing'
import { useQuery } from '@tanstack/react-query'
import type { Listing } from '@/lib/types'

interface ListingViewProps {
    listing: Listing & { user: any }
    currentUserId?: string
}

export function ListingView({ listing, currentUserId }: ListingViewProps) {
    ...
    useEffect(() => {
        if (id && !viewIncremented.current) {
            ...
            supabase.rpc('increment_listing_views', { listing_id: id }).then(({ error }: { error: Error | null }) => {
                    ...
                    <div className="p-5 space-y-6 md:py-8">
                        ...
                        {listing.location && (
                            <div className="text-xs text-muted-foreground flex items-center gap-1">
                                ...
                                {listing.location || "Ubicación aproximada"}
                            </div>
                        )}
