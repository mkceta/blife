...
import { ThreadList } from '@/features/messages/components/thread-list'
...
export function MessageList({ searchQuery = '' }: MessageListProps) {
    ...
    const { data: unreadCounts = {} } = useQuery({
        ...
        queryFn: async () => {
            ...
            unreadData?.forEach((msg: { thread_id: string }) => {
    ...
    // Only show skeleton on absolute first load or during hydration to match server
    if (!isMounted || (!filteredThreads.length && isPending)) return <MessagesSkeleton />
