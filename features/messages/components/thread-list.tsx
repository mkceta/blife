...
export const ThreadList = memo(function ThreadList({ initialThreads, currentUserId, unreadCounts: initialUnreadCounts }: ThreadListProps) {
    ...
    useEffect(() => {
        const channel = supabase
            ...
            .on('postgres_changes', {
                ...
                schema: 'public',
                table: 'users',
            }, (payload: { new: { id: string; last_seen?: string } }) => {
        ...
        <div className="space-y-2">
            {threads.map((thread, index) => {
                ...
                const latestMessage = thread.messages && thread.messages.length > 0
                    ? thread.messages.sort((a: { created_at: string }, b: { created_at: string }) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0]
                    : null
                ...
                return (
                    <div
                        key={thread.id}
                        className={index < 5 ? "animate-in fade-in-0 slide-in-from-bottom-2" : ""}
